local color = require "scripts.shared.third_party.color"
...
local function split_line(line, settings, words)
	...
	if trimmed_text == "" then
		...
		-- for word in gmatch(trimmed_text, "[^%p%d%s%c]+") do
		--for word in gmatch(trimmed_text, "%S+") do
...
function M.parse(text, word_settings)
	...
	repeat
		...
		--end
		for k, v in pairs(word_settings.tags or {}) do
			tag_settings.tags[k] = (params == "") and true or v
		end
