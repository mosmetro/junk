local parser = require "scripts.shared.third_party.parse"
local utf8 =require("scripts.shared.third_party.utf8")
...
local function create_text_node(word, font)
	...
	-- local c = vmath.vector4(word.color.x, word.color.y, word.color.z, 0)
...
function M.create(text, font, settings)
	...
	for i=1,#words do
		...
		local font_ = get_font(word, settings.fonts)

		-- create node and get metrics
		word.node, word.metrics = create_node(word, settings.parent, font_)
