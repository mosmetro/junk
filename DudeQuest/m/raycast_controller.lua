...
local RAY_LENGTH = 15 -- must be > (dt * max convergence speed)
local SKIN_THIN = 0.001
local SKIN_THICK = 0.001
...
local function new(owner)
   ...
   function controller.check_ledge(direction, pos_x, pos_y, dy, shift_y)
      ...
      local hit = raycast(ray_start, ray_end, owner.CLIMBABLE) --SOLIDS
      if hit then
         ...
         -- if abs(hit_distance * direction + other_dx) < SKIN_THIN then
            return nil, hit.id, dy -- ledge_id, wall_id, dy
         end
      end
      ...
      if hit then
         return nil, nil, dy -- ledge_id, wall_id, dy
      end
      ...
      vector3_set_components(ray_end, x, end_y)
      -- msg.post("@render:", "draw_line", { start_point = ray_start, end_point = ray_end, color = colors.WHITE })
      hit = raycast(ray_start, ray_end, owner.CLIMBABLE) -- GROUND
      if hit then
         local inst = get_instance(hit.id)
         -- if not inst.can_climb_up then
         --    return nil, nil, dy
         -- end
      ...
      return ledge_id, nil, dy -- ledge_id, wall_id, dy
      ...
      for _ = 1, horizontal_rays_count do
         ...
         if hit and hit.group ~= owner.SLOPE then
            ...
            if hit_distance < abs(delta) then
               ...
               if owner.on_contact then -- ?
                  vx = owner.on_contact(hit.id, direction, vx)
                  -- utils.log(runtime.current_frame, r, hit.id, direction, vx)
               end
      ...
      for i = start, finish, step do
         ...
         if other_dy < 0 then
            if dy < 0 then
               if hit_distance < abs(dy + other_dy) then
                  if hit.contact then
                     if owner.on_ground_contact and (step == 1) then
                        vy = owner.on_ground_contact(vy, hit.group, hit.id)
                     else
                        vy = 0
                     end
                  end
   ...
   local function down_pass(pos_x, pos_y, dx, dy, vy)
      ...
      local hit_id, hit_dx, hit_dy, hit_can_jump_down, hit_max_speed, hit_acceleration
            ...
            if ((nx == 0) or (ray == 1 and nx > 0) or (ray == vertical_rays_count and nx < 0)) and (hit.id ~= owner.bypass) then
               ...
               if hit.id ~= hit_id then
                  ...
                  if inst then
                     ...
                     hit_max_speed = inst.max_speed or INFINITY
                     hit_acceleration = inst.acceleration or INFINITY
                  else
                     hit_dx, hit_dy, hit_can_jump_down, hit_max_speed, hit_acceleration = 0, 0, false, INFINITY, INFINITY
                  end
               end
               local hit_distance = ensure_zero(hit.fraction * RAY_LENGTH - SKIN_THICK)
               hit.ray = ray
               ...
               hit.max_speed = hit_max_speed
               hit.acceleration = hit_acceleration
                  ...
                  if ground_hit ~= hit then
                     if (hit.dy > ground_hit.dy) then
                        ...
                        if hit.can_jump_down == ground_hit.can_jump_down then
                           local abs_hit_dx = abs(hit.dx)
                           local abs_ground_hit_dx = abs(ground_hit.dx)
                           if abs_hit_dx == abs_ground_hit_dx then
                              if hit.max_speed == ground_hit.max_speed then
                                 if hit.acceleration > ground_hit.acceleration then
                                    ground_hit = hit
                                 end
                              elseif hit.max_speed > ground_hit.max_speed then
                                 ground_hit = hit
                              end
                           elseif abs_hit_dx < abs_ground_hit_dx then
                              ground_hit = hit
                           end
         ...
         if hit then
            ...
            if hit_distance < (dy - other_dy) then
               ...
               if owner.on_contact_up then
                  owner.on_contact_up(hit.group, hit.id, dy)
               end
               if vy > 0 then
                  vy = 0
               end
      ...
      if hit then
         ...
         if current_slope_angle < MAX_ASCEND_ANGLE then
            ...
            if hit_distance < abs_dx then
               ...
               if owner.on_slope_contact then
                  vy = owner.on_slope_contact(vy)
               else
                  vy = 0
               end
         ...
         if (nx == 0) or (sign(nx) == direction) then
            ...
            if hit_distance <= proposed_dy then
               ...
               if hit then
                  ...
                  local next_slope_angle = ensure_zero(angle_between(hit.normal, UP))
                  -- utils.log(current_slope_angle, next_slope_angle)
                  hit_distance = ensure_zero(hit.fraction * RAY_LENGTH - SKIN_THIN)
                  if current_slope_angle == 0 then
                     ...
                     if -hit_distance > dy or combined_is_equal(-hit_distance, dy) then -- hit_distance < abs(dy)
                        ...
                        -- log("DY correction")
                     elseif next_slope_angle ~= 0 then
                        -- utils.log("second")
                        end_y = pos_y + dy
                        ...
                        hit = raycast(ray_start, ray_end, owner.GROUND)
                        if hit then
                           -- msg.post("@render:", "draw_line", { start_point = ray_start, end_point = ray_end, color = colors.BLUE })
                           -- utils.log("DX correction")
                           -- utils.log("before", dx)
