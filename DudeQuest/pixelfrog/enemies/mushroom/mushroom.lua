...
local drop_loot = loot_distributor.drop_loot
...
-- local loot_velocity_x_roll = fastmath.uniform_int(-70, 70)
-- local loot_velocity_y_roll = fastmath.uniform_int(160, 230)
...
local IDLE = {
   { id = hash("mushroom_idle"), position = vmath.vector3(0, 16, 0), },
}
local RUN = {
   { id = hash("mushroom_run"), position = vmath.vector3(0, 16, 0), },
}
local HIT = {
   { id = hash("mushroom_hit"), position = vmath.vector3(0, 16, 0), },
}
...
local function make()
   ...
   -- local drop_silver_coin
   -- local drop_blue_gem
   -- local drop_green_gem
   -- local drop_red_gem
   -- local drop_heart
   local loot = {
      drop_silver_coin = false,
      drop_gold_coin = false,
      drop_gem_blue = false,
      drop_gem_green = false,
      drop_gem_red = false,
      drop_heart = false,
   }
   ...
   function dead.on_enter()
      ...
      factory.create(factories.EFFECT_CIRCLE_PUFF, vector3_stub, QUAT_IDENTITY, nil, 1)
      drop_loot(loot, vector3_stub)
      -- if drop_silver_coin then
      --    params[ROOT].velocity_x = loot_velocity_x_roll()
      --    params[ROOT].velocity_y = loot_velocity_y_roll()
      --    collectionfactory.create(factories.COIN_SILVER, vector3_stub, QUAT_IDENTITY, params, 1)
      -- end
      -- if drop_gold_coin then
      --    params[ROOT].velocity_x = loot_velocity_x_roll()
      --    params[ROOT].velocity_y = loot_velocity_y_roll()
      --    collectionfactory.create(factories.COIN_GOLD, vector3_stub, QUAT_IDENTITY, params, 1)
      -- end
      -- if drop_blue_gem then
      --    params[ROOT].velocity_x = loot_velocity_x_roll()
      --    params[ROOT].velocity_y = loot_velocity_y_roll()
      --    collectionfactory.create(factories.GEM_BLUE, vector3_stub, QUAT_IDENTITY, params, 1)
      -- end
      -- if drop_green_gem then
      --    params[ROOT].velocity_x = loot_velocity_x_roll()
      --    params[ROOT].velocity_y = loot_velocity_y_roll()
      --    collectionfactory.create(factories.GEM_GREEN, vector3_stub, QUAT_IDENTITY, params, 1)
      -- end
      -- if drop_red_gem then
      --    params[ROOT].velocity_x = loot_velocity_x_roll()
      --    params[ROOT].velocity_y = loot_velocity_y_roll()
      --    collectionfactory.create(factories.GEM_RED, vector3_stub, QUAT_IDENTITY, params, 1)
      -- end
      -- if drop_heart then
      --    params[ROOT].velocity_x = loot_velocity_x_roll()
      --    params[ROOT].velocity_y = loot_velocity_y_roll()
      --    collectionfactory.create(factories.HEART, vector3_stub, QUAT_IDENTITY, params, 1)
      -- end
   ...
   function instance.init(self)
      ...
      loot.drop_silver_coin = self.drop_silver_coin
      loot.drop_gold_coin = self.drop_gold_coin
      loot.drop_blue_gem = self.drop_blue_gem
      loot.drop_green_gem = self.drop_green_gem
      loot.drop_red_gem = self.drop_red_gem
      loot.drop_heart = self.drop_heart
      -- drop_silver_coin = self.drop_silver_coin
      -- drop_gold_coin = self.drop_gold_coin
      -- drop_blue_gem = self.drop_blue_gem
      -- drop_green_gem = self.drop_green_gem
      -- drop_red_gem = self.drop_red_gem
      -- drop_heart = self.drop_heart
