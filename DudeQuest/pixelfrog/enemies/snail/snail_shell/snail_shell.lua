...
local MAX_HORIZONTAL_SPEED = 200
...
local IDLE = {
   { id = hash("shell_idle"), position = vmath.vector3(0, 11, 0), },
}
local RUN = {
   { id = hash("shell_run"), position = vmath.vector3(0, 11, 0), },
}
local HIT = {
   { id = hash("shell_hit"), position = vmath.vector3(0, 10, 0), },
}
local WALL_HIT = {
   { id = hash("shell_wall_hit"), position = vmath.vector3(0, 11, 0), },
}
...
local function make()
   ...
   function idle.on_enter()
      ...
      wake_time = runtime.current_time + 0.5
   end -- idle.on_enter

   function idle.update(dt)
      if runtime.current_time > wake_time then
   ...
   function turn.on_enter()
      ...
      play_animation(char, WALL_HIT)
      nc.post_notification(const.CAMERA_SHAKE_REQUEST_NOTIFICATION, nil, 3, 0.1)
      -- (1) turn duration
      next_run_time = runtime.current_time + 0.1
   end -- turn.on_enter
   ...
   function instance.on_collision(other_instance)
      if machine.current_state() == idle then
         msg.post(collisionobject_hitbox, msg.DISABLE)
         msg.post(collisionobject_hurtbox, msg.DISABLE)
         machine.enter_state(run)
         return
   ...
   function instance.init(self)
      ...
      contact_right = false
      turnaround(char)
      go.set_rotation(self.initial_rotation, char.anchor)
      go.animate(char.anchor, const.EULER_Z, go.PLAYBACK_ONCE_FORWARD, 0, go.EASING_LINEAR, 0.3)
